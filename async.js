...
function sayHowdy() {
  ...
  function limitedRepeat() {
    // ADD CODE HERE
    let myVar = setInterval ( function() {console.log("hi for now");}, 1000)
  ...
  function delayCounter(target, wait) {
    let counter = 1;
      ...
      let intervalFunc = setInterval ( function () { console.log(counter); counter++; }, wait);
  ...
  function promised (val) {
    return new Promise(resolve=> setTimeout( function() {resolve(val)}, 2000));
  }
  ...
  class SecondClock {
    ...
    start () {
      this.timer = setInterval( () =>{ this.curVal++;
                                                        this.cb(this.curVal); } ,1000)
    }
    reset() {
  ...
  function debounce(callback, interval) {
    ...
    return function () {
      if(!bouncer)
        {
          bouncer =1;
          isActiveTimer = setTimeout(function () { bouncer = 0}, interval)
          return callback();
        }
      else
        {
