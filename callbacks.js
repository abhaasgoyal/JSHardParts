...
function reduce(array, callback, initialValue) {
  ...
  forEach(array, (value) => { finalValue = callback(value,finalValue);})
	return finalValue;
}
...
function intersection(...arrays) {
  const reduceIntersect = (curArr,inputArr) => curArr.filter((value) => inputArr.indexOf(value) !== -1 ) 
...
function objOfMatches(array1, array2, callback) {
  ...
  const forEachFunc = (value) => {
    if(array2.includes(callback(value)))
      newObj[value] = callback(value)
  } 
...
function multiMap(arrVals, arrCallbacks) {
  ...
  const forEachNameFunc = (arrName) => {
    ...
    const forEachValFunc = (arrCallback) => tempArrCallbackProp.push(arrCallback(arrName))
...
function majority(array, callback) {
	let trueInstances = 0, falseInstances = 0;
